...
class JournalManager {
    ...
    async init() {
        // Aggiorna la data compatta nell'header
    ...
    updateCompactDate() {
        ...
        const dateEl = document.getElementById('tecDateCompact');
        
        if (dateEl) {
            // Formato: "Sunday 07/12/56" (prima lettera maiuscola, resto minuscolo)
            const dayName = currentDate.dayName; // Prende già "Sunday" dal TEC
            const dayNum = currentDate.date.getDate().toString().padStart(2, '0');
            const monthNum = (currentDate.date.getMonth() + 1).toString().padStart(2, '0');
            const year = currentDate.year;
            
            dateEl.innerHTML = `
                <span class="tec-day">${dayName}</span>
    ...
    async loadJournal() {
        try {
            ...
            // Verifica se aggiungere la data di oggi
    ...
    checkAndAddTodayHeader() {
        ...
        // Semplicemente: data TEC su una riga nuova
        const todayHeader = `\n\n${tecDate.formatted}\n`;
        ...
        // Cerca la data TEC di oggi nell'ultima parte
