...
function convertDate(dateStr) {
  try {
    const date = new Date(dateStr + 'T12:00:00Z') // UTC noon to avoid time zone problems
...
function getYearInfo(yearStr) {
  ...
  if (isNaN(year) || year < 0) {
    return jsonResponse({ 
      error: 'Invalid year',
      note: 'The year must be a positive number (0 = 1969)' 
    }, 400)
  }
...
function getSeptenniumInfo(septStr) {
  ...
  if (isNaN(sept) || sept < 1) {
    return jsonResponse({ 
      error: 'Invalid Septennium',
      note: 'The Septenniums start from 1' 
    }, 400)
  }
